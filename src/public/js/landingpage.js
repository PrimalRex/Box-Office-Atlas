...
function updateLandingPageWithMovieData(data) {
    ...
    document.querySelector('#movie-subbox').innerText = `${data.movie.movieDetails.earliestReleaseDate || "TBC"} // ${data.movie.movieDetails.runningTime || "TBC"} // ${data.movie.movieDetails.mpaa || "NOT RATED YET"}`;
    ...
    document.querySelector('#movie-info-table td:nth-child(2)').style.color = 'hsl(240, 4%, 20%)';
    document.querySelector('#movie-info-table tr:nth-child(2) td:nth-child(2)').style.color = 'hsl(240, 4%, 20%)';
    ...
    for (let i = 0; i < geoData.length; i++) {
        ...
        countryFinancial.innerText = geoData[i].countryGross + " // " + geoData[i].countryGrossPercentage + "%";
        ...
        marker.getElement().addEventListener('mouseenter', () => {
            marker.getElement().querySelector('#marker-country-financial').classList.add('visible');
            marker.getElement().querySelector('#marker-country').classList.add('visible');
        ...
        marker.getElement().addEventListener('mouseleave', () => {
            marker.getElement().querySelector('#marker-country-financial').classList.remove('visible');
            marker.getElement().querySelector('#marker-country').classList.remove('visible');
